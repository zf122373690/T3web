...
function defaultConfig(config?: T3Config): T3Config {
  return {
    ...
    mqtt: {
      ...
      topicPrefix: config?.mqtt?.topicPrefix || 'sms',
      keepAlive: config?.mqtt?.keepAlive || 60,
      statusInterval: config?.mqtt?.statusInterval || 60,
    },
          ...
          {!takeoverDevice ? (
            ...
            <>
              ...
              <div className="takeover-grid">
                ...
                <section className="takeover-card wide"><h3><Radio size={17} /> MQTT 公网配置</h3><div className="takeover-two"><label>MQTT 开关<select value={config.mqtt?.enabled ? 1 : 0} onChange={(event) => patchConfig({mqtt: {...config.mqtt, enabled: event.target.value === '1'}})}><option value={1}>开启</option><option value={0}>关闭</option></select></label><label>Broker 地址<input value={config.mqtt?.broker || ''} onChange={(event) => patchConfig({mqtt: {...config.mqtt, broker: event.target.value}})} placeholder="例如 mqtt.example.com" /></label></div><div className="takeover-two"><label>端口<input type="number" value={config.mqtt?.port || 1883} onChange={(event) => patchConfig({mqtt: {...config.mqtt, port: Number(event.target.value)}})} /></label><label>主题前缀<input value={config.mqtt?.topicPrefix || 'sms'} onChange={(event) => patchConfig({mqtt: {...config.mqtt, topicPrefix: event.target.value}})} placeholder="sms" /></label></div><div className="takeover-two"><label>用户名<input value={config.mqtt?.username || ''} onChange={(event) => patchConfig({mqtt: {...config.mqtt, username: event.target.value}})} /></label><label>密码<input type="password" value={config.mqtt?.password || ''} onChange={(event) => patchConfig({mqtt: {...config.mqtt, password: event.target.value}})} /></label></div><div className="takeover-two"><label>心跳(秒)<input type="number" value={config.mqtt?.keepAlive || 60} onChange={(event) => patchConfig({mqtt: {...config.mqtt, keepAlive: Number(event.target.value)}})} /></label><label>状态上报间隔(秒)<input type="number" value={config.mqtt?.statusInterval || 60} onChange={(event) => patchConfig({mqtt: {...config.mqtt, statusInterval: Number(event.target.value)}})} /></label></div><label>Client ID<input value={config.mqtt?.clientId || ''} onChange={(event) => patchConfig({mqtt: {...config.mqtt, clientId: event.target.value}})} placeholder="留空自动生成" /></label><label>本地上报地址<textarea value={config.localUrl || ''} onChange={(event) => patchConfig({localUrl: event.target.value})} /></label></section>
